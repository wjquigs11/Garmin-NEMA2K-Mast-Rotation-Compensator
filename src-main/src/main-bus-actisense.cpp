...
HardwareSerial ESPlink(2);  // UART2

Stream *read_stream = &ESPlink;
...
void HandleStreamActisenseMsg(const tN2kMsg &message) {
  ...
  if (ParseN2kWindSpeed(message,SID, windSpeedMetersSeconds, windAngle, WindReference) ) {
...
void readSendESPwind() {
  ...
  Serial.print(" angle: ");
  Serial.println(angle.floatP);
  float windSpeed = ESPlink.parseFloat();
  int windAngle = ESPlink.parseInt();
  Serial.print("speed: ");
...
void setup() {
  ...
  ESPlink.begin(115200, SERIAL_8N1, RXD0, TXD0); 
  if (!ESPlink)
    Serial.println("failed to open ESPlink!");
  ...
  app.onRepeat(10, []() {
    ...
    actisense_reader.ParseMessages(); // should pick up Wind
